...
const ProjectBoardFilters = ({
  ...
  const setFiltersMerge = newFilters =>
    setFilters({ ...filters, ...newFilters });

  const areFiltersCleared =
    !searchQuery && userIds.length === 0 && !myOnly && !recent;
  ...
  return (
    <Filters>
      <SearchInput
        icon="search"
        onChange={debounce(
          value => setFiltersMerge({ searchQuery: value }),
          500
        )}
      />
      <Avatars>
        {projectUsers.map(user => (
          ...
          >
            <StyledAvatar
              ...
              name={user.name}
              onClick={() =>
                setFiltersMerge({ userIds: xor(userIds, [user.id]) })
              }
            />
          </AvatarIsActiveBorder>
        ))}
      ...
      {!areFiltersCleared && (
        <ClearAll onClick={() => setFilters(defaultFilters)}>
          Clear all
        </ClearAll>
      )}
    </Filters>
