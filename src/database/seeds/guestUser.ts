...
// seed Issues (return them using Promise.all)
const seedIssues = (project: Project): Promise<Issue[]> => {
  ...
  const issues = [
    ...
    createEntity(Issue, {
      title: 'You must assign priority from lowest to highest on all issues.',
      type: IssueType.TASK,
...
const seedComment = (issue: Issue): Promise<Comment> =>
  createEntity(Comment, {
    body: 'Be nice to each other!',
    issue,
    user: issue.users[0],
  });
...
const seedGuestUserEntities = async (user: User): Promise<void> => {
  ...
  await seedComment(issues[issues.length - 1]);
};
